...
class GridWorld(Environment):
    ...
    # def get_reduced_observation(self, handle):
    #     reduced_view_space = (self.view_space[handle.value][0] // 2, self.view_space[handle.value][1] // 2)
    #     reduced_feature_space = (self.feature_space[handle.value][0] // 2,)
    #     no = handle.value
    #     n = self.get_num(handle) // 2
    #     view_buf = self._get_obs_buf(no, self.OBS_INDEX_VIEW, (n,) + reduced_view_space, np.float32)
    #     feature_buf = self._get_obs_buf(no, self.OBS_INDEX_HP, (n,) + reduced_feature_space, np.float32)
    #
    #     bufs = (ctypes.POINTER(ctypes.c_float) * 2)()
    #     bufs[0] = as_float_c_array(view_buf)
    #     bufs[1] = as_float_c_array(feature_buf)
